...
export function TimerDisplay({ routine, state }: Props) {
  ...
  return (
    <section className="my-4 grid content-center gap-3 rounded-lg border border-[#f1c29b] bg-[#fffdfa] px-4 py-8 text-center shadow-xl shadow-[#d96a1f]/10">
      ...
      <div className="mt-2 rounded-lg border border-[#f1c29b] bg-[#fff7ef] p-3 text-left" aria-label="現在の位置">
        <div className="mb-2 flex items-center justify-between gap-3">
          <h2 className="m-0 text-sm font-black text-[#8a4b23]">現在の位置</h2>
          <span className="rounded-full bg-[#f26a21] px-2.5 py-1 text-xs font-extrabold text-white">{progress}%</span>
        </div>
        <div className="grid gap-1.5">
          {visibleItems.map(({ item, index }) => {
            ...
            return (
              ...
              >
                <span className={`grid h-6 w-6 place-items-center rounded-full text-xs font-black ${isCurrent ? 'bg-[#f26a21] text-white' : isDone ? 'bg-[#ead8c7] text-[#6d5a4d]' : 'bg-[#fff0df] text-[#b84b12]'}`}>
                  {isDone ? '✓' : index + 1}
                </span>
                <span className={`truncate ${isCurrent ? 'font-black' : 'font-bold'}`}>{item.title}</span>
                <span className={`rounded-full px-2 py-0.5 text-xs font-extrabold ${item.type === 'interval' ? 'bg-[#e7f5f3] text-[#12675d]' : 'bg-[#fff0df] text-[#b84b12]'}`}>{item.durationSec}秒</span>
              </div>
            );
          })}
        </div>
